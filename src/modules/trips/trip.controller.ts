...
export class TripController {
  ...
  async seatSelection(
    ...
    const seatLayout = await this.tripSeatService.buildSeatLayout(
      trip?.id,
      trip.bus?.id,
      this.getUserIdFromCookie(req),
    );
    const basePrice = Number(trip.promotionalValue ?? trip.valor ?? 0);
  ...
  async reserveSeat(
    ...
    const userId = (req as Request & { user?: { id: number } }).user?.id;
    const reservation = await this.paymentService.reserveSeat(
      userId!,
      tripId,
      String(seat),
    );
  ...
  async releaseSeat(
    ...
    const userId = (req as Request & { user?: { id: number } }).user?.id;
    const reservation = await this.paymentService.releaseSeatReservation(
      userId!,
      tripId,
      String(seat),
    );
